...
# --- 1. Impor Bobot yang Sudah Ditanamkan ---
from embedded_weights import weights # <-- PERUBAHAN: Impor bobot dari file baru
...
NUM_CLASSES = 4
DEVICE = "cpu"
# MODEL_PATH tidak lagi dibutuhkan

# --- 2. Arsitektur Model U-Net (Tidak ada perubahan) ---
...
# --- 3. Fungsi Helper & Preprocessing ---

# <-- PERUBAHAN: Fungsi load_model sekarang memuat dari variabel, bukan file
@st.cache_resource
def load_model_from_embedded_weights():
    """
    Membuat instance model dan memuat bobot dari variabel Python yang diimpor.
    """
    # Konversi kembali dictionary of lists menjadi dictionary of tensors
    state_dict = {key: torch.tensor(val, dtype=torch.float32) for key, val in weights.items()}
    
    # Buat instance arsitektur model
    model = UNET(in_channels=1, out_channels=NUM_CLASSES).to(DEVICE)
    
    # Muat bobot ke dalam model
...
def main():
    ...
    with st.sidebar:
        ...
        st.image("https://i.imgur.com/CVQ8J47.png", width=150)
    ...
    # --- Halaman Utama (Sedikit perubahan pada pemuatan model) ---
    ...
    with st.container(border=True):
        uploaded_file = st.file_uploader("Pilih file gambar...", type=["png", "jpg", "jpeg"], label_visibility="collapsed")
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("🧠 Menganalisis gambar dan melakukan segmentasi..."):
            # <-- PERUBAHAN: Memanggil fungsi load model yang baru
            model = load_model_from_embedded_weights()
        ...
        # Sisa kode untuk menampilkan hasil tidak ada perubahan
    ...
    with st.expander("ℹ️ Lihat Detail Teknis Model"):
        ...
        - **Model:** U-Net
        - **Framework:** PyTorch
        - **Bobot Model:** Tertanam langsung di dalam kode (embedded).
